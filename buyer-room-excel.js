(function (root) {
  ...
  function zip(files) {
    ...
    for (const [name, content] of files) {
      const nameBytes = encoder.encode(name), bytes = encoder.encode(content), crc = crc32(bytes), start = offset;
  ...
  function buildSelectionExcel(data) {
    ...
    const sheet = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><dimension ref="A1:G${endRow}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="2" topLeftCell="A3" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A3" sqref="A3"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="18"/><cols><col min="1" max="1" width="19" customWidth="1"/><col min="2" max="2" width="52" customWidth="1"/><col min="3" max="3" width="27" customWidth="1"/><col min="4" max="4" width="10" customWidth="1"/><col min="5" max="6" width="16" customWidth="1"/><col min="7" max="7" width="15" customWidth="1"/></cols><sheetData>${header}${body}</sheetData><autoFilter ref="A2:G${endRow}"/><hyperlinks><hyperlink ref="A1" r:id="rId1" tooltip="Open your Buyer Room"/></hyperlinks></worksheet>`;
  ...
  function downloadSelectionExcel(data) {
